...
`/tmpl-setup` Step 2 and `/tmpl-bootstrap` Step 0 — see those files for
...
1. **Plugin manifest** — verify `../ai-plugins.json` exists and
   ...
   - If `../ai-plugins.json` is missing, stop and tell the user:
     > `ai-plugins.json` is missing. Run `/tmpl-bootstrap` first to generate it.
   ...
   - If any path is missing, list them and ask the user whether
     to halt, run `/tmpl-verify` to auto-fix, or proceed anyway.

2. **Inherit `skipped_prereqs[]`** — read
   `configure.json.skipped_prereqs[]` (set by `/tmpl-setup` Step 0
...
- New source code lives **only inside paths declared in
  ...
  genuinely needs a new module, halt and ask the user to re-run `/tmpl-setup`
  to declare it.
...
to `/tmp/claude-tmpl-release-new-{run-id}/{relative-path}`, open them in
...
- The Step 5 task-list approval is always required before Step 6
  begins. `/tmpl-release-new` is the project's task-protocol checkpoint —
...
- Step 6 skipped (per-task) → mark the task as `skipped` in
  ...
  release summary so `/tmpl-release-finish` knows.
- Step 7 skipped (first release only) → leave the README's Setup
  placeholder in place; tell the user they can re-run `/tmpl-release-new`
  Step 7 manually or hand-edit `setup.sh` / `run.sh` later.

Re-runs of `/tmpl-release-new` against an existing release with skipped
...
`project-summary.md` written by `/tmpl-setup` Step 5a and is read-only
context for every subsequent release. Reject `init` as a release name
(say so explicitly: "`init` is reserved for `/tmpl-setup`'s project
...
- Every new file lands under a `path` declared in
  ...
  `/tmpl-setup` to declare it first.
...
detection rules as `/tmpl-setup` Step 5's "How files are presented
...
`/tmpl-setup` Step 5a wrote a placeholder Setup section in `README.md` at
...
(the bootstrap-owned block between the `<!-- /tmpl-bootstrap appends ... -->`
markers belongs to `/tmpl-bootstrap`, and everything else belongs to
`/tmpl-setup`).
