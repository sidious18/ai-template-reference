# Command: /tmpl-release-delete [name]
...
this command's gating**. `/tmpl-release-delete` is destructive end-to-end —
...
1. **Plugin manifest** — verify `../ai-plugins.json` exists and
   every path resolves on disk. Same gate as `/tmpl-release-new`,
   `/tmpl-release-edit`, and `/tmpl-release-finish` — destructive operations
   ...
   - If any path is missing, list them and ask the user whether
     to halt, run `/tmpl-verify` to auto-fix, or proceed
...
`/tmpl-release-delete` is destructive. Unlike `/tmpl-setup`, `/tmpl-bootstrap`,
`/tmpl-release-new`, `/tmpl-release-edit`, and `/tmpl-release-finish`, this command
...
3. **Step 3 Option 2 — Revert confirmation with editor diff
   ...
   as `/tmpl-setup` Step 5's "How files are presented for review"
   ...
   - **Files about to be reverted** (`git checkout` target) —
     ...
     pre-revert content to `/tmp/claude-tmpl-release-delete-{run-id}/{path}`
...
The `init` release is reserved for `/tmpl-setup`'s project summary
(`releases/init/project-summary.md`) and **cannot be deleted** by this
command. If the user names `init`, refuse with: "`init` is the
project-summary release written by `/tmpl-setup` and is required by
`/tmpl-bootstrap`. Re-run `/tmpl-setup` if you need to regenerate it." Do not
...
6. Clean up the staging directory at
   `/tmp/claude-tmpl-release-delete-{run-id}/`.
