...
as `/new-release`. If `approval_rate_overrides.finish_release` is set
...
1. **Plugin manifest** — same as `/new-release` Step 0. Halt if
...
`/tmp/claude-finish-release-{run-id}/{relative-path}`, open them in
...
remaining subtasks before closing), that branch follows `/new-release`
...
- The `init` release **cannot be finished** by this command. If the
  ...
  written by `/configure`; it has no task list to verify and no
  meaningful status to set." (Mirrors `/delete-release`'s `init`
  guard.)

**Skip handling.** When the user picks **Skip** for a gated write:
- Step 4 skipped → no `summary.md` written; tell the user the release
  is still marked unfinished and `/finish-release` can be re-run
  later.
- Step 5 skipped → no `status: done` marker written; the release
  stays Active in `/list-releases`.
...
`finish-release:{release-name}:{step-id}` — e.g.,
`finish-release:checkout-flow:6a`) so re-runs and `/analyze` can
see what was deferred. Same convention as `/configure`'s
...
match the `^status: done$` regex on its own line that `/list-releases`
...
`/configure` set up. Run the following sub-steps **only when the
...
The Project Overview page (created in `/configure` Step 5m) has a
...
When the release has an open PR (typical when `/new-release` Step
8b created one), post a comment marking the release done with the
same summary used in 6a. Use the GitHub MCP. Preview, then `yes` /
`edit` / `skip` / `abort`. Do **not** auto-merge — that is the
team's call.
