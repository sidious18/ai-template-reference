# Command: /tmpl-analyze [free-form focus, optional]
...
`/tmpl-setup` would have generated, what has drifted (if both states
exist), and which follow-up commands would close each gap.

`/tmpl-analyze` is **never destructive**. It does not write `ai-plugins.json`,
...
include in the final implementation plan; `/tmpl-analyze` then prints a
tailored, copy-pasteable sequence of follow-up commands the user
runs themselves. `/tmpl-analyze` never executes any of them — that
guarantee is the whole reason `/tmpl-analyze` is safe to run anywhere
(unfamiliar repo, production checkout, sandbox).

## When to use

- You inherited an existing codebase and want a baseline of what's there
- You ran `/tmpl-setup` long ago and want a drift report
- You want to preview what `/tmpl-setup` would do before committing to it
- You want a concrete checklist of `/tmpl-reconfigure` invocations to pick from

If the user supplied a free-form focus argument
(`/tmpl-analyze CI workflows`, `/tmpl-analyze missing role docs`,
`/tmpl-analyze Confluence drift`), narrow the report to that area. Without
...
not halt** if either is missing. `/tmpl-analyze` runs in any environment;
...
- `CLAUDE.md` — present at project root and **not** the template
  ...
  if found, the project never ran `/tmpl-bootstrap`).
- `AGENTS.md`, `AI_INSTRUCTIONS.md` — present in `ai-instructions/`.

---

## Step 3: Artifact inventory (mirror of `/tmpl-setup` Step 5)

For each `/tmpl-setup` Step 5 sub-section, mark present / partial /
...
- `.github/workflows/ai-pr-review.yml` (Full when `integrations.github.rules.prs.ai_review.provider` ≠ `none`)
- `.github/labeler.yml`
...
**AI instruction pack** (`/tmpl-bootstrap` outputs)
...
When `configure.json` is missing, infer the values `/tmpl-setup` would
have captured. Present them as **starting points the user would
confirm or edit during a future `/tmpl-setup` run**, not as decisions.
...
- `team_roles[]` — empty by default, with **suggestions** based on
  ...
  conversation prompts for `/tmpl-setup`, not defaults.
...
- **Layout drift:** directories on disk not in `layout.modules[]`
  ...
  no longer exists (stale entries). Same check `/tmpl-reconfigure` Step
  2 runs.
...
- **Skipped-prereq drift:** any prerequisite recorded in
  ...
  Confluence + Jira disabled. Install and re-run `/tmpl-setup` to
...
goal is a complete, granular suggestion list that mirrors `/tmpl-setup`
Step 5 — every artifact and every external-system action that
`/tmpl-setup` would have produced shows up here as one suggestion the
user can pick or skip individually.

| Finding kind | Suggested action |
|---|---|
| `configure.json` missing entirely | Run `/tmpl-setup full` (or `/tmpl-setup core` / `/tmpl-setup custom`) |
| Git baseline missing (`.gitignore`, `.gitattributes`, `.editorconfig`) | `/tmpl-reconfigure "add git baseline files"` |
| Hooks missing (`.pre-commit-config.yaml` / `.husky/`, commit-msg validation) | `/tmpl-reconfigure "add commit hooks"` |
| Gitflow + onboarding docs missing (`docs/gitflow.md`, `docs/onboarding.md`) | `/tmpl-reconfigure "add gitflow + onboarding docs"` |
| Per-language conventions missing (`docs/conventions/{lang}.md`) | `/tmpl-reconfigure "add language conventions docs"` |
| Per-role onboarding docs missing (`docs/roles/{role}.md`) | `/tmpl-reconfigure "regenerate role onboarding docs"` |
| Full-scope governance missing (`SECURITY.md`, `CONTRIBUTING.md`, `LICENSE`, `docs/code-review.md`) | `/tmpl-reconfigure "add governance docs"` |
| GitHub artifacts missing (CODEOWNERS, dependabot, PR/issue templates, CI, labeler) | `/tmpl-reconfigure "add GitHub artifacts"` |
| AI PR review workflow missing (`.github/workflows/ai-pr-review.yml`) — flag when `integrations.github.rules.prs.ai_review.provider` is not `none` (or unset and the user wants it) | `/tmpl-reconfigure "enable AI PR review with {github-models\|anthropic-api\|bedrock\|vertex}"` (default: `github-models` — free, no setup) |
| Release automation missing (release-please / changesets) | `/tmpl-reconfigure "add release automation"` |
| Branch protection rules missing on default branch | `/tmpl-reconfigure "set branch protection rules"` (requires GitHub MCP) |
| Confluence pages missing (Project Overview / Requirements / Technologies / User Roles) | `/tmpl-reconfigure "create Confluence pages"` (requires Atlassian MCP + space key + page-create permission on that space) |
| Tech Lead review Jira ticket missing | `/tmpl-reconfigure "open Tech Lead review ticket"` (requires Atlassian MCP + Jira project key + create-issue permission) |
| Per-role onboarding Jira tickets missing | `/tmpl-reconfigure "open role-onboarding tickets"` (requires Atlassian MCP + Jira project key) |
| Specific Step 5 sub-section missing in a configured project | `/tmpl-reconfigure "add {artifact name}"` — e.g., `/tmpl-reconfigure "add CI workflows"` |
| Manifest drift (file gone, plugin still enabled) | Run `/tmpl-verify`, then `/tmpl-reconfigure` to reconcile |
| Layout drift (new directory not declared) | `/tmpl-reconfigure "add {path} as a {kind} module"` |
| Stale `team_roles[]` (no role for declared module kind) | `/tmpl-reconfigure "add a {role} role"` |
| Skipped item the user may want to revisit | "Recorded as skipped on {date}; re-run `/tmpl-reconfigure 'redo {item}'` if you want it now" |
| MCP-only check that wasn't reachable | "Install the {GitHub\|Atlassian} MCP and re-run `/tmpl-analyze` to check this" |
...
| Finding kind | Suggested action |
|---|---|
| Track this analysis as a Jira ticket | `/tmpl-reconfigure "open Jira ticket for /tmpl-analyze findings on {YYYY-MM-DD}, assigned to {runner}"` (requires Atlassian MCP + create-issue permission). The ticket body includes the picked-action checklist below; the runner is the user invoking `/tmpl-analyze` (Atlassian `accountId` from `mcp__*atlassian*__atlassianUserInfo`). |
...
> ## `/tmpl-analyze` report — {project name or path} ({date})
>
...
> - Install the missing MCP server(s) and re-run `/tmpl-analyze` to include these.
...
> [ ] 9a. Add `.github/workflows/ai-pr-review.yml` (AI PR review — provider: github-models / anthropic-api / bedrock / vertex) — only when `integrations.github.rules.prs.ai_review.provider` ≠ `none` OR the user picks it now
...
> [ ] 20. Run `/tmpl-verify` to reconcile manifest
>
...
selected — rerun `/tmpl-analyze` any time" note.
...
- **Multiple `/tmpl-reconfigure` picks against the same project** can
  often combine into one invocation: `/tmpl-reconfigure "add git
...
- **For greenfield (no `configure.json`)**, suggest `/tmpl-setup
  custom` instead of stitching `/tmpl-reconfigure` calls — `/tmpl-setup`
  knows how to ask the upfront questions that `/tmpl-reconfigure`
...
>
> Run these commands in order. `/tmpl-analyze` did not execute any of
...
>
> ```
> /tmpl-reconfigure "add git baseline + commit hooks + gitflow + onboarding docs + per-language conventions + GitHub artifacts (CODEOWNERS, dependabot, PR/issue templates, ci/labeler/ai-pr-review workflows) + release-please"
> ```
>
...
>
> ```
> /tmpl-reconfigure "set branch protection rules on main: required PR review, status checks (ci, pr-title-check), no force-push"
> ```
>
> **3. Confluence + Jira** *(Atlassian MCP)*
>
> ```
> /tmpl-reconfigure "create Confluence pages (Project Overview, Requirements, Technologies, User Roles) on space ENG"
> /tmpl-reconfigure "open Tech Lead review ticket on PLAT, assignee {tech-lead-from-configure.json}"
> /tmpl-reconfigure "open role-onboarding tickets on PLAT, one per team_roles[]"
> /tmpl-reconfigure "open Jira tracking ticket on PLAT for /tmpl-analyze findings on {YYYY-MM-DD}, assigned to {runner display name + accountId}, body = picked-action checklist below"
> ```
>
...
> ```
> /tmpl-verify
> /tmpl-reconfigure "remove stale web-ui module from layout"
> ```
>
> **Skipped because prerequisite missing:**
> - (none — all prerequisites available for picked items)
>
> **What you didn't pick** — re-run `/tmpl-analyze` any time to revisit
...
`/tmpl-analyze` writes nothing else. The report file (if saved) goes to the
release `init/` directory because it's project-context, not a code
release.

---

## What `/tmpl-analyze` never does
...
- Never invokes `/tmpl-setup`, `/tmpl-bootstrap`, `/tmpl-reconfigure`, or any
...
production repo, or a sandboxed copy. Re-running `/tmpl-analyze` produces
