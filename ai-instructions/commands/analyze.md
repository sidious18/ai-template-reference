...
| AI PR review workflow missing (`.github/workflows/ai-pr-review.yml`) — flag when `integrations.github.rules.prs.ai_review.provider` is not `none` (or unset and the user wants it) | `/edit-config "enable AI PR review with {github-models\|anthropic-api\|bedrock\|vertex}"` (default: `github-models` — free, no setup) |
