...
`/configure` is the first command run on a new project, before
`/bootstrap`. It collects every up-front decision (stack, code conventions,
...
`/bootstrap` consumes the decision record and generates the AI instruction
pack. `/new-release` then produces application code.

Order of operations on a new project:

    /configure   →   /bootstrap   →   /new-release
...
`/configure` (and every later command — `/bootstrap`, `/new-release`,
`/edit-release`, `/finish-release`, `/edit-config`) depends on a small
...
| 4 | **IDE MCP** (`mcp__ide__executeCode`, `mcp__ide__getDiagnostics`) | optional | both tools listed in the session | live diagnostics + in-IDE code execution during `/new-release` validation |
...
3. **On failure** — present a single decision block:
   ...
   > **abort** (halt the whole `/configure` run).
...
6. **On `abort`** — halt the whole `/configure` run. Preserve any
...
`/configure` opens an initial PR in Step 5n, so the working tree must
be ready to push.
...
- Working tree dirty with **unrelated** changes (i.e., not from a
  prior partial `/configure` run) → ask the user to commit or stash
  first.
...
>   live)
>
> Re-run `/configure` after installing the Atlassian MCP to switch
> back to live mode.
...
- Install / enable the **Claude Code** extension from the
  marketplace.
- Reload the editor and run `/configure` from the **integrated
...
> - Live in-editor diagnostics during `/new-release` validation
...
write it to `ai-plugins.json` during `/bootstrap`).
...
`/bootstrap` reads `prereqs` to know which integration files to
...
MCP after `/configure` finished), re-running `/configure` re-probes
...
or anything else. `/configure` owns all requirement gathering for the
project.
...
1. Read every file in **all conventional requirements locations**, in
   ...
   - `ai-instructions/releases/` — the canonical post-`/configure`
     home (this is where `/configure` itself writes
   ...
   - Anything the user explicitly named when invoking `/configure`
...
post-`/configure` home — the user can keep their existing
...
rest of `/configure` (and every later command) behaves.
...
> How much do you want `/configure` to set up?
>
...
> Discovery mode for `/new-release` and friends:
>
...
How often should `/configure` (and later commands) pause for the user
...
`/configure` will show what it's about to send and wait for
confirmation even when `approval_rate: auto`.

Save as `approval_rate`. This propagates into `ai-plugins.json`
during `/bootstrap` Step 5 and is read by every later command that
writes files.
...
provider lists must agree, and `/configure` flags the mismatch if
they don't.
...
enforcement rule below (questions 46, 23, 45, 47) accordingly — no
...
| # | Question | Default | Scope |
|---|---|---|---|
| 53 | Enable automated AI code review on every PR (Claude) | **yes** | full |
| 54 | AI review provider | **(suggested from `deployment.target`: AWS → `bedrock`, GCP → `vertex`, else → `anthropic-api`)** — one of `bedrock`, `anthropic-api`, `vertex` | full (asked only when #53 is yes) |
| 55 | Claude model for review | **(provider-specific default; see Step 5g.1)** | full (asked only when #53 is yes) |

In Core scope, default `ai_review.enabled` to `false` silently — Core
is supposed to stay minimal. Custom scope follows the user's per-question
picks like every other Full-scope question.

When #53 is yes, the workflow generated in Step 5g.1 fires on
`pull_request: [opened, synchronize, reopened]` and posts (or updates)
a single sticky review comment per PR. The configure PR opened in Step 5n
is reviewed the same way — Step 5n prints the secret/OIDC setup
instructions for the chosen provider **before** pushing the branch so the
first run succeeds.
...
that `/bootstrap` enables under `ai-instructions/roles/`. Every team role
...
`onboarding_doc` (boolean). The `name` slug is what `/bootstrap` uses
...
static HTML styled to resemble Confluence — questions 54 and 55 do not
apply and are skipped.

| # | Question | Default | Scope | Local-docs fallback |
|---|---|---|---|---|
| 53 | Create project docs | **yes** | core | same |
| 54 | Confluence space key | **(pulled from the MCP spaces list; ask if more than one is accessible)** | core | **skipped** |
| 55 | Parent page under which to create new pages | **space root** | core | **skipped** |
| 56 | Page set to create | **Project Overview, Requirements, Technologies, User Roles** (the basic set) | core | same |
| 57 | Additional pages | **none** — optional: Gitflow, Onboarding (copies of `docs/gitflow.md` + `docs/onboarding.md`) | full | same |
| 58 | Keep the pages in sync on later `/configure` edits | **yes** — update rather than re-create | full | same (regenerate the HTML files in place) |
...
what `/configure` actually captured.
        ...
        "github": {
          ...
          "rules": {
            ...
            "prs": {
              ...
              "ai_review": {
                "comment": "Output of questions #53–#55. When enabled=true, Step 5g.1 generates .github/workflows/ai-pr-review.yml and Step 5n prints the secret/OIDC setup instructions for the chosen provider. Set enabled=false (and omit provider/model/region) to skip the workflow entirely. The provider must match where the team has Claude access — `bedrock` for AWS, `anthropic-api` for direct Anthropic API keys, `vertex` for GCP. `model` is the provider-specific Claude model id used for review (cheaper Sonnet is the default; Opus only when the team explicitly opts in for higher review quality). `region` is required for `bedrock` and `vertex` and **must be omitted** for `anthropic-api`. The optional `secrets_pending: true` flag (added in Step 5n only when the user defers secret setup) signals that the workflow file is committed but the runtime secrets haven't been set yet.",
                "enabled": true,
                "provider": "bedrock",
...
6. **Render `html-prototype` screens to PNGs (when applicable).**
   ...
   **Render each screen.** Use Playwright (preferred — gives
   ...
   3. **No tooling at all** — surface the gap explicitly and
      ...
      > and re-run `/configure`. In a downstream project that has
...
nested forms only for `/bootstrap` consumers that haven't been
migrated.
...
1. **Stage the proposal.** Write the proposed content to
   `/tmp/claude-configure-{run-id}/{relative-path}` (where `{run-id}`
...
4. **Apply the action.**
   ...
   - **abort** → `rm -rf /tmp/claude-configure-{run-id}/`, halt.
5. **Final cleanup.** At the end of the `/configure` run, remove the
   staging directory.

For modifications to existing files (e.g., a re-run of `/configure`
...
(e.g., `["5h", "5o"]`). On a re-run of `/configure`, do not re-prompt
...
read by `/bootstrap` and the release commands so they know which
...
ground-truth for `/bootstrap` Step 5 and for every future release's
...
of `/configure` (or `/edit-config "sync project summary"`) read the
...
above. `/bootstrap` will append a Quickstart / AI Workflow section
...
Structure:
    ...
    - **AI workflow:** [docs/ai-workflow.md](docs/ai-workflow.md) — what the slash commands do and how the AI pack fits together (written by `/bootstrap` Step 8b)
...
`/new-release` has run), say so explicitly: "These scripts will
exist after the first `/new-release` runs. Until then, the manual
...
During `/configure` ask once: "What's the channel/email new
...
### 5g.1. AI PR review workflow (Full + Custom when #53 is yes)

Generated only when `integrations.github.rules.prs.ai_review.enabled` is
`true`. Skip the entire sub-section otherwise.

Writes one workflow file: `.github/workflows/ai-pr-review.yml`. The
workflow uses the official `anthropics/claude-code-action@v1`. It
triggers on `pull_request: [opened, synchronize, reopened]` so the
review runs once when the PR opens and re-runs on every push, posts (or
updates) a single sticky review comment per PR via
`use_sticky_comment: "true"`, and is read-only with respect to repo
contents.

**Permissions block** (identical across providers):
...
`id-token: write` is required for Bedrock and Vertex (OIDC); harmless
for the direct Anthropic API path. Keep it on for all three so that
switching provider via `/edit-config` later doesn't require a workflow
rewrite.

**Review prompt** (identical across providers — written into the
workflow's `prompt:` input):

```text
You are reviewing this pull request for {project.name}.

Produce ONE comment with these sections:

1. **Summary** — 2–3 sentences on what this change does.
2. **Findings** — concrete issues only. Cite file:line. Group by:
   - Correctness / bugs
   - Security
   - Performance
   - Maintainability / readability
   Skip stylistic nitpicks already enforced by linters.
3. **Suggestions** — actionable changes the author can make. Quote
   the diff snippet you'd change.
4. **Verdict** — one of: approve · request-changes · comment-only.

Project conventions live in `docs/conventions/` and `CLAUDE.md` — honor
them when those files are present. The configure PR (the workflow's
first run) is reviewing template-generated artifacts, so those files
may not exist yet; in that case, focus on whether the artifacts look
internally consistent.
```

**Placeholder substitution rule.** Before writing
`.github/workflows/ai-pr-review.yml`, substitute every `{...}`
placeholder in **both** the YAML examples below and the prompt above
with the literal value from `configure.json`:

| Placeholder | Source |
|---|---|
| `{project.name}` | `project.name` |
| `{ai_review.model}` | `integrations.github.rules.prs.ai_review.model` |
| `{ai_review.region}` | `integrations.github.rules.prs.ai_review.region` (Bedrock + Vertex only) |
| `{ai_review.region_prefix}` | derived from region geo: `us-*` → `us`, `eu-*` → `eu`, `ap-*` → `apac` (Bedrock cross-region inference profiles only — only used when the model id explicitly contains `{ai_review.region_prefix}.`; the default Bedrock model id below already pins to `us.` so no derivation is needed for the default) |

Failing to substitute leaves placeholders verbatim in the workflow
file, which makes the GitHub Actions YAML invalid and the run fails
on the first dispatch.

**Provider-specific generation.** Pick the variant matching
`integrations.github.rules.prs.ai_review.provider`.

> **Model-id sanity check before writing.** The default model ids below
> reflect the latest Claude Sonnet known at template-release time. They
> can drift — a new Sonnet ships, a provider changes the id format, an
> account doesn't have access to the cross-region inference profile.
> Before writing the workflow, the AI must:
>
> 1. If question #55 was answered explicitly, use that value verbatim.
> 2. Otherwise, use the variant default below.
> 3. Either way, surface the chosen model id to the user as part of
>    Step 5n's pre-push prompt: "AI review will use `{ai_review.model}`
>    — confirm this id exists in your account, or override now."
>
> This is a one-line confirmation, not a probe. The CI run is the
> source of truth — if the id is wrong, the first PR-review job fails
> with a clear "model not found" error and `/edit-config` can swap it.

#### Variant A — Bedrock (`provider: "bedrock"`)

Default model when the user did not override #55:
`us.anthropic.claude-sonnet-4-6-20251001-v1:0` (US cross-region
inference profile). Default region: `us-west-2`. Both come from
`ai_review.model` and `ai_review.region` respectively in `configure.json`.
  ...
  review:
    ...
    steps:
      ...
      - name: Configure AWS credentials (OIDC)
        ...
        with:
          role-to-assume: ${{ secrets.AI_REVIEW_AWS_ROLE_ARN }}
          aws-region: {ai_review.region}
      ...
      - name: Run Claude PR review
        ...
        with:
          ...
          prompt: |
            {review prompt from above, with {project.name} substituted}
```

#### Variant B — Direct Anthropic API (`provider: "anthropic-api"`)

Default model when the user did not override #55: `claude-sonnet-4-6`.
...
#### Variant C — Vertex AI (`provider: "vertex"`)

Default model when the user did not override #55:
`claude-sonnet-4-6@20251001`. Default region:
`us-east5` (the canonical Claude-on-Vertex region). The Vertex project
id lives in the `AI_REVIEW_GCP_PROJECT_ID` repository secret (set in
Step 5n alongside the WIF provider and service account secrets).
    ...
    steps:
      ...
      - name: Run Claude PR review
        ...
        env:
          ...
          CLOUD_ML_REGION: {ai_review.region}
        with:
          use_vertex: "true"
          use_sticky_comment: "true"
          claude_args: |
...
outage should never block merges. Treat the AI review as advisory.
The user can opt in by editing `branch_protection.required_checks`
manually.

**Failure handling.** If the secret(s) referenced by the chosen
variant are missing when the workflow first runs, GitHub Actions
fails the job with a clear "Missing secret" message but the PR
itself is unaffected. Step 5n (below) is responsible for printing
the exact secret-setup commands so the user can complete this
**before** the configure PR is opened — when they do, the workflow
runs successfully on the configure PR itself.

For a GitLab repo, the equivalent is a `.gitlab-ci.yml` job that
calls the same model via the provider's REST API and posts the
review through the GitLab MR Notes API. Document this as a TODO when
`ci.provider != "github-actions"` rather than generating a half-working
workflow.
...
| Tech Lead | Every `docs/conventions/*.md`; `ai-plugins.json`; every active guide | Review `ai-plugins.json` (run `/verify-plugins`); review CI workflows under `.github/workflows/`; read every release's `requirements.md` in `ai-instructions/releases/` |
...
- **User Roles** — a Mermaid org-chart-style diagram if `team_roles[]`
  has clear hierarchy, or a swimlane / responsibility matrix if it
  doesn't. Pulls labels from `team_roles[].display`.
...
- **Remote (Confluence MCP)** — when
  ...
  `/configure` run, update the existing page rather than creating a
  duplicate.
  ...
  **Failure handling:**
  ...
  - If the call fails partway through the page set (e.g., 3 of 4 pages
    ...
    later `/configure` re-run with `sync_on_edit: true` will pick up
...
- **Local-docs fallback** — when
  ...
  styled to resemble a Confluence page. File layout:
      ...
      ├── technologies.html
      ├── user-roles.html
      └── {optional}.html            # gitflow, onboarding (question 57)
        ...
        <div class="page-layout">
          ...
          <main class="page-content">
            ...
            <footer class="page-meta">
              Generated by /configure on {ISO date}. Edit and regenerate
              in place on the next /configure edit run.
            </footer>
          </main>
        </div>
      </body>
...
and where do I go next" page — not a stripped-down summary.
    ...
    /finish-release Step 6b runs, this section grows one row per
    finished release: name, date, short summary, link to PR.
    Auto-managed by /finish-release; manual edits are overwritten
    on the next run.}
...
reference, not just a list of names.
    ...
    regenerated from that file on every `/configure` run."
    ...
    **CI workflows** — link to each workflow file in the repo
    ...
    Edit those, then re-run `/configure` (or `/edit-config "sync
...
fidelity, not an abbreviated summary.
    ...
    > below are regenerated from those files on every `/configure`
    ...
    storage-format markup but otherwise identical. Specifically,
    every section produced by 5j shows up here:
    ...
    > then re-run `/configure` (or `/edit-config "sync user roles
...
**Optional pages** when question #57 is yes:
...
> Default: the user running `/configure`.
...
1. **Create the Jira ticket** via Atlassian MCP
   ...
   - Type: `Task`
   - Summary: `Review /configure output for {project.name}`
   - Description (markdown in Jira's native format):
     ```
     /configure has prepared the initial project setup. Please review:
...
2. **Create the branch + commit** locally via `git`:
   ...
   - Stage every file produced by this `/configure` run. Include
     ...
     `ai-pr-review.yml` when `ai_review.enabled`, plus release
   ...
   - Commit with Conventional Commits. Include the ticket ID when Jira is
     enabled; omit it otherwise:
         ...
         # Jira enabled
         chore: {KEY}-{NUMBER} initial project setup via /configure

         # Jira disabled (local-docs fallback)
         chore: initial project setup via /configure
...
3. **Push the branch** to `origin`.
   ...
   **Before this push**, if
   `integrations.github.rules.prs.ai_review.enabled` is `true`,
   **halt and print the secret-setup instructions for the chosen
   provider, then wait for the user to confirm the secrets are in
   place.** The workflow generated in Step 5g.1 will fire on the
   configure PR itself the moment the branch is pushed; without the
   secrets the first run fails noisily. Print exactly the variant
   matching `ai_review.provider`:

   - **bedrock** —
     > The AI PR review workflow needs an AWS IAM role assumable via
     > GitHub OIDC, with permission to call
     > `bedrock:InvokeModel` and
     > `bedrock:InvokeModelWithResponseStream` on the model
     > `{ai_review.model}` in region `{ai_review.region}`.
     >
     > 1. Create (or reuse) an IAM role with that policy and a trust
     >    relationship for `token.actions.githubusercontent.com`
     >    scoped to `repo:{owner}/{repo}:*`.
     > 2. Add the role ARN as a repository secret named
     >    `AI_REVIEW_AWS_ROLE_ARN`:
     >
     >        gh secret set AI_REVIEW_AWS_ROLE_ARN --body "arn:aws:iam::<acct>:role/<role>"
     >
     > Confirm both are done, then I'll push the branch.
   ...
   - **anthropic-api** —
     ...
     >
     > Confirm done, then I'll push the branch.
   ...
   If the user opts to skip secret setup right now (e.g., they need
   to coordinate with their cloud admin), still push the branch and
   open the PR, but **be explicit about the consequence**: the
   `ai-pr-review.yml` workflow file is already committed, so GitHub
   will run it on every PR (starting with the configure PR itself)
   and the run will fail with a clear "Missing secret" error until
   the secrets are added. The PR itself merges fine because the
   workflow is not in `branch_protection.required_checks`, but a red
   X next to every PR is noisy. Two acceptable shapes:

   - **Default — leave the workflow file in.** Record the deferred
     state by adding the nested `secrets_pending: true` flag under
     `ai_review` (i.e. set
     `configure.json → integrations → github → rules → prs →
     ai_review → secrets_pending = true`).
     Mention in the PR body (at the bottom of the review checklist):
     "AI review workflow is committed but inactive until the
     `{provider-specific secret list}` are set; runs will show as
     failed checks until then."
   - **Alternative — un-commit the workflow.** When the user
     ...
     before pushing, set `ai_review.enabled = false`, and tell them
     to re-enable later via
     `/edit-config "enable AI PR review with {provider}"`.

   Skip this entire conditional when `ai_review.enabled` was already
   `false` going in.
...
- **Description** (Jira native markup):
      ...
      Welcome. /configure has prepared a setup for the {display} role.
...
3. Present the next-step summary:
   ...
   > After they approve and merge, run `/bootstrap` on the updated default
   > branch. `/bootstrap` reads `configure.json` and generates the AI
