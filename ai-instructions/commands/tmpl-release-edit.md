# Command: /tmpl-release-edit [name]
...
Same semantics as `/tmpl-release-new`. If
...
2. **Inherit `skipped_prereqs[]`** — read
   ...
   is identical to `/tmpl-release-new` Step 0:
...
- New or moved code lives inside the `path` of some entry in
  ...
  if the edit genuinely needs one, re-run `/tmpl-setup` to declare it first.
...
`/tmp/claude-tmpl-release-edit-{run-id}/{relative-path}`, open them in
...
`tmpl-release-edit:{release-name}:{step-id-or-relative-path}`) so re-runs
and `/tmpl-analyze` can see what was deferred. Same convention as
`/tmpl-setup`'s `skipped[]` tracking. Re-runs re-prompt the gate
...
If the release does not exist, say so and suggest `/tmpl-release-new` instead.
