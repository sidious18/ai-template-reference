...
**This is not `/finish-release`.** `/finish-release` finishes a
feature release of a downstream project bootstrapped from this
template; `/release` cuts a new version of the template itself.
...
2. **Branch** — `git rev-parse --abbrev-ref HEAD` must be `main`.
   Halt otherwise: "/release runs from main only. Switch and rerun."
...
`/tmp/claude-release-{run-id}/{relative-path}`, open `VERSION` as
...
- `git push origin main` rejected (someone else pushed first) →
  halt; tell the user to pull and re-run `/release`. The local
...
Clean up the staging dir at `/tmp/claude-release-{run-id}/`.
