...
must include the two `<!-- /bootstrap: stack-guides start ({slug}) -->`
and `<!-- /bootstrap: stack-guides end ({slug}) -->` markers, even
when the role pulls no stack-specific guides (e.g., `tech-writer`,
`library-author`, `security-engineer`, `platform-engineer`, `sre`,
`devops-engineer`). For these roles, leave the block empty between the
markers — `/bootstrap` Step 2b's filter table tells the generator
which roles get content and which stay empty. Missing markers cause
Step 2b to log a warning and skip population for that role.
...
resolves on disk before referencing — `/verify-plugins` will catch
