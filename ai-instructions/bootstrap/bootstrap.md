...
stack. It runs on `/bootstrap` or first session.

`/bootstrap` configures the AI instruction pack. It does not implement any
code — code implementation starts with `/new-release`.

`/bootstrap` is a pure generator. Its only inputs are
`ai-instructions/configure.json` and
`ai-instructions/releases/init/project-summary.md` (both produced by
`/configure`). Throughout the rest of this file, paths starting with
...
- `/configure` must have been run. If `configure.json` is missing,
  `/bootstrap` halts — see Step 1.
...
from `ai-plugins.json.layout` (which `/bootstrap` writes from
`configure.json.layout`, in turn populated during `/configure`'s
...
- The set of modules is decided during `/configure` (no defaults — see
  Step 3a of `commands/configure.md`). Common shapes:
...
## Step 0: Inherit Operating Modes from `/configure`

`mode` and `approval_rate` were captured by the most recent
`/configure` run (Step 2b + 2c) and live in
...
Do **not** re-ask the user. Re-asking duplicates `/configure`'s
...
1. **`configure.json` is missing.** Halt — `/configure` must run
   first (this is also enforced by Step 1's hard requirement).
   When the user explicitly skipped `/configure` to write a manual
...
2. **`configure.json` is missing one or both fields** (older
   `/configure` runs from before these settings existed). Ask the
   single missing field once, with the same options as
   `/configure` Step 2b/2c, and write back to `configure.json` for
   future runs.

3. **The user invoked `/bootstrap --reconfigure` explicitly** —
...
These are the same enums `/configure` uses; bootstrap honors them
verbatim.
...
## Step 1: Require `/configure` output (HARD REQUIREMENT)

Check whether `../configure.json` exists at the project root.

**If it is missing, halt and tell the user:**

> `/bootstrap` requires `ai-instructions/configure.json`, which is produced
> by `/configure`. Requirement discovery, stack decisions, and gitflow
> choices are made by `/configure`.
>
> Run `/configure` first, then re-run `/bootstrap`.

`/bootstrap` does not ask questions and does not perform its own discovery.
...
- `approval_rate: per-category` — at the **start** of each step
  ...
  `commands/configure.md` Step 5 ("How files are presented for
  review") — the same staging path convention
  (`/tmp/claude-bootstrap-{run-id}/{relative-path}`), the same editor
...
There are no MCP-side-effect steps in `/bootstrap` itself (those live
in `/configure`), so no mandatory gates. Pure local file writes only.
...
`team_roles[]` — **whatever the user declared during /configure, no
...
them in `team_roles[]` during `/configure`.
...
block looks like:
    ...
    <!-- /bootstrap: stack-guides start ({role-slug}) -->
    - `../guides/{guide}.md`
    - `../guides/{another-guide}.md`
    <!-- /bootstrap: stack-guides end ({role-slug}) -->
...
| `devops-engineer` | no stack-guides block by default — ops guides are not stack-organized; leave the marker block empty (`<!-- /bootstrap: stack-guides start (devops-engineer) -->\n<!-- /bootstrap: stack-guides end (devops-engineer) -->`) |
| `qa-engineer`, `business-analyst`, `ui-ux-designer` | no stack-guides block — these roles are stack-agnostic. Skip population |
...
- `custom` → if the user's original `/configure` answers requested detailed
...
during `/configure`. Bootstrap does not second-guess that choice.
...
Write it as JSON with this shape:

    {
      ...
      "commands": [
        { "name": "configure", "path": ".claude/commands/configure.md" },
        { "name": "bootstrap", "path": ".claude/commands/bootstrap.md" },
        { "name": "verify-plugins", "path": ".claude/commands/verify-plugins.md" },
        { "name": "analyze", "path": ".claude/commands/analyze.md" },
        { "name": "edit-config", "path": ".claude/commands/edit-config.md" },
        { "name": "new-release", "path": ".claude/commands/new-release.md" },
        { "name": "edit-release", "path": ".claude/commands/edit-release.md" },
        { "name": "finish-release", "path": ".claude/commands/finish-release.md" },
        { "name": "delete-release", "path": ".claude/commands/delete-release.md" },
        { "name": "list-releases", "path": ".claude/commands/list-releases.md" },
        { "name": "list-active-releases", "path": ".claude/commands/list-active-releases.md" }
      ]
    }
...
- `plugins.*` — one entry per role/guide/guideline/refactoring doc you either
  ...
  so `/verify-plugins` can tell the two apart and so future regenerations
...
- `layout` — copy `kind`, `modules[]`, and `exceptions[]` from
  ...
  modules** the user did not declare; if Step 3a in `/configure`
  recorded a single-module project, do not split it into backend/frontend
  here.
- `layout.exceptions` — same array `/configure` produced. Each exception
...
`ai-plugins.json` is re-verified at the start of every `/new-release`,
`/edit-release`, and `/finish-release` run. If verification fails at that
point, the command halts and asks the user to re-run `/bootstrap` or fix the
manifest manually.
...
| `{SETUP_COMMAND}`, `{DEV_COMMAND}` | Always `./setup.sh` and `./run.sh`. Generated during the first `/new-release` (Step 7 of `commands/new-release.md`). Do not substitute raw framework commands here. |
...
| `{DESCRIBE_DI_PATTERNS,_ERROR_HANDLING,_TESTING_APPROACH,_STORAGE_APPROACH}` | Distill from `docs/conventions/{lang}.md` (written by `/configure` Step 5k) plus stack-specific guides under `ai-instructions/guides/`. |
...
1. **Loading Order** — ensure the numbered list under "How To Use This Pack"
   ...
   gitflow, role docs, conventions — produced by `/configure`), then repo
...
6. **Project Docs** — add a short section pointing at `../docs/` with entries
   for every file produced by `/configure` and `/bootstrap`:
...
2. **Where to find what** — add entries for all generated files, including
   ...
   by `/configure`, and **`docs/ai-workflow.md`** (written by Step 8b).
...
are already on disk by Step 5).
    ...
    - Adding a feature → `/new-release [name]`
    - Editing an existing feature → `/edit-release [name]`
    - Finalizing and marking a feature done → `/finish-release [name]`
    - Auditing project state → `/analyze`
    - Adjusting setup (stack, roles, integrations, …) → `/edit-config`
    - Sanity-checking the AI manifest after manual edits → `/verify-plugins`
    ...
    1. **`/configure`** — collected the project's decisions: stack,
    ...
    2. **`/bootstrap`** — read the decision record and generated the
    ...
    3. **`/new-release`** — implemented the first feature. The first
       run also wrote `setup.sh` + `run.sh`. Every subsequent feature
       is another `/new-release`.
    4. **`/edit-config`** — used later to change setup decisions
    ...
    - `setup.sh` / `run.sh` — the project's only canonical entry
      points (written by the first `/new-release`).
    ...
    - `roles/{role-slug}.md` — per-role onboarding doc (one per
      `team_roles[]` entry from `/configure`).
    ...
    - `AI_INSTRUCTIONS.md` — task protocol, output rules, loading
      priorities.
    - `configure.json` — decision record, written by `/configure`.
    ...
    1. Open the AI tool. It auto-loads `CLAUDE.md`, then
       ...
       `ai-settings.md`).
    2. Type a slash command (`/new-release`, `/edit-release`, …).
    ...
    `/configure` and `/edit-config` regenerate this file on every run
    so the "Active configuration" section above stays accurate. If
    you edit the AI instruction files manually (e.g., flip a
    checkbox in `ai-settings.md`), re-run `/edit-config` or
    `/verify-plugins` to keep the human-readable doc in sync. If you
...
markers — re-running `/bootstrap` overwrites it cleanly.

---

## Step 9: Update README.md (Quickstart + AI Workflow)

`/configure` Step 5a wrote a project-facing `README.md` at the root and
...
Output (markdown, written verbatim — substitute the bracketed values):
    ...
    `/new-release` and `/edit-config` walk the AI through structured
    ...
    **[docs/ai-workflow.md](docs/ai-workflow.md)** — what each
    ...
    - `/new-release [name]` — start a new feature.
    - `/edit-release [name]` — modify an existing release.
    - `/finish-release [name]` — finalize and mark a release done.
    - `/list-releases` / `/list-active-releases` — show release status.
    - `/edit-config` — change setup decisions (stack, roles, etc.).
    - `/analyze` — read-only project audit.
    - `/verify-plugins` — sanity-check `ai-plugins.json`.
    ...
    Run `/new-release` to scaffold the first feature. That command
...
content above the opening marker (that belongs to `/configure`).

If `README.md` is missing entirely (e.g., the user deleted it after
`/configure` ran), regenerate the `/configure` portion first by
...
4. **Present summary**:
   ...
   > - Run `/new-release` to start your first feature
...
The initial requirements gathered by `/configure` Step 1 are saved to
