...
Selection`. `/tmpl-bootstrap` Step 8
...
testing guidelines). `/tmpl-bootstrap` Step 8 rewrites this section to list
...
The list below is the **template's starter sample**. `/tmpl-setup`
...
docs, 5k per-language conventions); `/tmpl-bootstrap` Step 8
...
The list below is the **template's starter sample** — eight common
roles from the 15-slug curated catalog at `AGENTS.md § Role
Selection`. `/tmpl-bootstrap` Step 8 rewrites this section so it lists
