---
description: Configure a new project — decisions, gitflow, hooks, CI, docs (run before /tmpl-bootstrap)
argument-hint: "[core|full|custom]"
---

Read `ai-instructions/commands/tmpl-setup.md` from the project root and follow
every step in order. Do not skip steps.

`/tmpl-setup` is the **first** command on a new project. It runs before
`/tmpl-bootstrap` and produces the decision record (`ai-instructions/configure.json`)
plus the real workflow artifacts (git config, hooks, CI workflows, onboarding
and gitflow docs, SECURITY / CONTRIBUTING, branch protection). `/tmpl-bootstrap`
reads `configure.json` and skips its own discovery.

`/tmpl-setup` is **discovery-driven, no defaults**: project kind, modules,
...
`ai-instructions/commands/tmpl-setup.md` — read them there.
