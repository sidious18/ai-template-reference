...
Read `ai-instructions/commands/tmpl-analyze.md` from the project root and
follow every step in order.

`/tmpl-analyze` is the read-only discovery counterpart to `/tmpl-setup`. It
surveys the project, presents every gap (mirroring `/tmpl-setup`'s full
...
themselves — that guarantee is the whole reason `/tmpl-analyze` is safe to
...
- You want to preview what `/tmpl-setup` would do before committing
- You want a checklist of `/tmpl-reconfigure` invocations to pick from
