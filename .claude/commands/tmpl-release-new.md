...
Read `ai-instructions/commands/tmpl-release-new.md` from the project root and follow
...
- New code lives **only inside paths declared in
  ...
  `/tmpl-setup`; do not invent backend/frontend or any other module the
  manifest doesn't list. If the project genuinely needs a new module,
  re-run `/tmpl-setup` to declare it before writing files.
